...
classnames = loadstr('./Classes/classes.txt', nameonly)
numexamples = loaddict('./Classes/numexamples.txt', int)
...
def evaluate(split, C, attributepattern, p_type='binary'):
    ...
    if split == 0:
        test_classnames = loadstr('./Classes/testclasses.txt')
        train_classnames = loadstr('./Classes/trainclasses.txt')
    else:
        startid = (split - 1) * 10
        stopid = split * 10
    ...
    M = np.loadtxt('./predicate-matrixs/' + 'predicate-matrix-' + p_type + '.txt', dtype=float)
...
def evaluate_(split, C, attributepattern, p_type='binary'):
    global test_classnames
    num = loaddict('./Classes/num.txt', int)
    if split == 0:
        test_classnames = loadstr('./Classes/testclasses.txt')
        train_classnames = loadstr('./Classes/trainclasses.txt')
    else:
